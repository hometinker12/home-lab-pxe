...
2. **Broaden with synonyms and globs** — When the target is described by behavior ("where is auth checked?") rather than a known identifier, grep for likely names (`require_`, `Depends(`, `session`) and use Glob to find candidate files by name; tighten filters when grep returns too many hits.
