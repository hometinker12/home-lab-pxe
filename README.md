...
Compose publishes HTTP `8080`, HTTPS `8443`, UDP `67` / `69` / `4011` (DHCP, TFTP, proxyDHCP), and NFS `2049` / `20048`. On a Linux lab host, add `network_mode: host` in `docker-compose.override.yml` so DHCP broadcasts, TFTP, SMB, and NFS see the LAN without NAT. Keep `PXE_BIND_INTERFACE` on the LAN NIC. Set `PXE_SMB_PASSWORD` in `.env` for Windows Setup media. Do not publish DHCP/TFTP/HTTP-boot to the internet.
