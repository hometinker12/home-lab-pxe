...
Docker-packaged PXE/iPXE boot server for a home lab. It discovers machines on the LAN. **Unknown** and **disabled** systems continue to the next boot device after a short timeout (no install menu). After you name a host in the console, the client sees a folder-based iPXE menu (Windows / Linux / Tools by default) with a countdown to local disk; picking an OS image starts that install. Linux installs get full [cloud-init](https://cloudinit.readthedocs.io/); Windows Server uses `unattend.xml` plus [Cloudbase-Init](https://cloudbase.it/cloudbase-init/). Linux **root** and Windows **local Administrator** credentials are stored encrypted. Changes you stage in the console apply on the **next PXE boot** as a new image.
...
- Console status is **Deploying**, **Imaging**, **Deployed**, **Disabled**, or **Timeout Error**. Imaging longer than the Settings timeout (default 15 minutes) becomes Timeout Error; Deploy again to retry. You can delete a machine from the list or its detail page; the next PXE boot with that MAC registers as unknown again.
- Upload Linux kernels/initrd, Windows WIM files, or Ubuntu/Windows Server ISO images in **Images** (kernel/initrd paths sit under Advanced Settings). ISO import shows upload progress, then returns to the image list while a background worker extracts netboot payloads. After a successful NFS (Ubuntu casper) or SMB (Windows Setup) extract, the uploaded ISO is deleted. You can delete an image from the list or its edit page. Each Linux image has editable cloud-init user-data; each Windows image has editable `unattend.xml`. Deploy saves hostname and guest-init from the same form, then copies the image template onto the machine if that file is still empty; **Copy Default** refreshes it from the image. Guest timezone is an IANA dropdown; new machines inherit **Settings → Machines** default timezone. Ubuntu live-server installs extract `casper/`, `.disk/`, `dists/`, and `pool/` into `images/nfs/{id}/{rev}` and boot with `netboot=nfs` (each extract is its own NFS export so the installer mounts squashfs instead of wget-ing the ISO into RAM). Autoinstall answers come from HTTP `/cloud-init/{machine}/user-data` at install time, not from the Files snapshot of the template. HTTP `iso-url=` remains the fallback when squashfs is missing. Windows Setup maps an authenticated read-only SMB share (Linux `network_mode: host` — Docker Desktop cannot publish LAN TCP 445). Stock Windows ISOs do not include Cloudbase-Init.
- **Boot menu** in the console builds the nested iPXE folders the client sees (default Windows, Linux, and Tools). Each image must live in a folder. Unknown and disabled hosts skip that menu and continue to disk after the configured timeout. Named or ready hosts get a countdown “continue to next boot device” item; choosing a Linux or Windows image starts a deploy/reimage. Tool images boot without changing machine state.
- Skip the install path only while a deploy is already in progress; otherwise a named host sees the folder menu instead of an immediate local-disk exit.
