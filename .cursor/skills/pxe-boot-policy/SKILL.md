...
3. **Deploying** or **staged reimage** → serve the assigned image:
   - Linux: kernel/initrd plus cloud-init seed URL. Ubuntu live-server with extracted casper squashfs uses `netboot=nfs nfsroot=host:/export` and `NFSOPTS=vers=3,tcp,port=2049` (HTTP `iso-url=` is fallback). Casper `nfsmount` still needs rpcbind on TCP/UDP 111 for mountd. Ganesha exports only the NFS root; each generation's `casper/` is hardlinked there because a nested export Path EPERMs on Linux and a subdirectory MNT is EACCES.
