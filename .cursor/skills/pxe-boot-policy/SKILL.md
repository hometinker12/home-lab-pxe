...
- UEFI boot order (machine **Next Boot Device**, `pxe` default / `disk`): Linux late-command order is phone-home → `/boot-files/uefi-boot-order.py` reorder → blocking `sync` + sysrq `b`. The reorder command must not contain `sysrq-trigger` or `reboot -f`. Windows appends a specialize `RunSynchronousCommand` (max Order + 1, after phone-home) to the existing `Microsoft-Windows-Deployment` component and never adds a second one. For `pxe`, the helper re-POSTs the events URL and only reorders on 200/409, so a lost phone-home cannot cause a reinstall loop.
